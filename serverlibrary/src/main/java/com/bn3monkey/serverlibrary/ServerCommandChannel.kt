...
import com.bn3monkey.serverlibrary.stream.PipeStream
import com.bn3monkey.serverlibrary.stream.PipeStreamFactory
...
class HansonoUltrasoundCommandChannel(
    parentScope : CoroutineScope
) {
    private val tag = "HansonoUltrasoundCommandChannel"
    ...
    fun open(nativeChannel : ServerNativeLibraryChannel) {
    ...
    suspend fun handleAccept(key : SelectionKey, selector : Selector)
    {
        ...
        Log.d(tag, "Command Channel connects with client (${clientChannel.remoteAddress})")
    }
    suspend fun handleRead(key : SelectionKey, buffer : ByteBuffer,
                           nativeChannel : ServerNativeLibraryChannel,
